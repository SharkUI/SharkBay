...
export class HookBridge extends EventEmitter<HookBridgeEvents> {
  ...
  private deployHookCli(appDataPath: string): void {
    ...
    // Main hook CLI (stdin-based agent hook protocols)
...
process.stdin.on("end", () => {
  ...
  const sharkbaySessionId = process.env.SHARKBAY_SESSION_ID || process.env.SHARKBAY_RESTORED_SESSION_ID;
  if (source === "reasonix" && sharkbaySessionId && parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
    parsed = { ...parsed, sharkbaySessionId };
  }
