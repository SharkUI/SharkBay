...
export class TerminalManager extends EventEmitter<TerminalManagerEvents> {
  ...
  async create(runtime: IpcRuntimeLike, input: TerminalCreateInput): Promise<TerminalSession> {
    ...
    if (input.agentId && initialCommand && !input.service && !isSessionRestore) {
      ...
      if (input.artifact) {
        const artifactPath = await reserveArtifactPath(spec.projectRoot, input.artifact.taskId);
        artifactPromptText = artifactPrompt({ ...input.artifact, artifactPath }, { codeGraphEnabled });
      }
