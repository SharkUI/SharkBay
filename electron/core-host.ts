...
const dispatchers: Record<CoreMethodName, Dispatcher> = {
  ...
  deleteProjectFile: ([runtime, input]) => core.deleteProjectFile(runtime as never, input as never),
  renameProjectFile: ([runtime, input]) => core.renameProjectFile(runtime as never, input as never),
  ...
  inspectTerminal: ([sessionId]) => core.inspectTerminal(sessionId as string),
  notifyTerminal: ([input]) => core.notifyTerminal(input as never),
