...
## Work
- Root cause: state was keyed by projectId — all agent tabs in same project shared one dot; idle was auto-cleared on project focus.
...
- Renderer DashboardView: hookStateBySessionId stores {state, projectId} per agent session; hookActivityByProjectId now derived (highest priority across sessions).
- Renderer TerminalPane: useMemo maps agentSessionId → terminalSessionId via heuristic; tab dot reads per-terminal state.
- Idle clear logic: only when user clicks on the idle tab, or terminal exits/closes. No more auto-clear on project focus.
- Project list pill derives from session states with priority aggregation (attention > working > idle).
...
- Stale mapping entries cleaned up on tab close via reverse lookup.
